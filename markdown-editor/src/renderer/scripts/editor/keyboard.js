...
(function(App) {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp' || e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
      ...
      if (selection.rangeCount > 0) {
        ...
        if (codeBlock && App.dom.editor.contains(codeBlock)) {
          ...
          if (inlineCode && App.dom.editor.contains(inlineCode)) {
            if (e.key === 'ArrowRight') {
              const atEnd = (node === inlineCode && range.startOffset >= inlineCode.childNodes.length) ||
                (node.nodeType === Node.TEXT_NODE && range.startOffset >= node.textContent.length &&
                 !hasNextSiblingInElement(node, inlineCode));

              if (atEnd) {
                e.preventDefault();
                const newRange = document.createRange();
                newRange.setStartAfter(inlineCode);
                newRange.collapse(true);
                selection.removeAllRanges();
                selection.addRange(newRange);
                App.dom.editor.focus();
            ...
            if (e.key === 'ArrowLeft') {
              const atStart = (node === inlineCode && range.startOffset <= 0) ||
                (node.nodeType === Node.TEXT_NODE && range.startOffset <= 0 &&
                 !hasPrevSiblingInElement(node, inlineCode));

              if (atStart) {
                e.preventDefault();
                const newRange = document.createRange();
                newRange.setStartBefore(inlineCode);
                newRange.collapse(true);
                selection.removeAllRanges();
                selection.addRange(newRange);
                App.dom.editor.focus();
  ...
  function hasNextSiblingInElement(node, ancestor) {
  ...
  function hasPrevSiblingInElement(node, ancestor) {
    while (node && node !== ancestor) {
